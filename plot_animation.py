...
def update(frame):
    ...
    ax.scatter(bfgs_vel[:frame], bfgs_err[:frame], c=np.arange(frame), s=np.arange(frame)+1, cmap=cmap, alpha=0.7,  label='BFGS Results')
...
# Save the animation
animation.save('data/bfgs_animation.gif', writer='imagemagick', fps=10)